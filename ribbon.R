...
ribbon <- function(x0, x1, y0, y1, h0, h1, stepsize = 0.05) {
  ...
  nsteps <- length(x)
  xx <- seq(-pi/2, pi/2, length.out = )
  
  ymin <- seq(y0, y1, length.out = length(x)) # this will change to a curve
  ymax <- seq(y0 + h0, y1 + h1, length.out = length(x)) # this will change to a curve
